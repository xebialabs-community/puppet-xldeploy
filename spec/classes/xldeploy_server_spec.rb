...
describe 'xldeploy::server' do
  ...
  shared_examples 'a Linux Os' do
    ...
    context 'xldeploy with version set to 4.5.0' do
      ...
      it { should contain_file('xldeploy server hotfix').with({:ensure=>'present',:owner=>'xldeploy',:group=>'xldeploy',:mode=>'0640',:ignore=>'.gitkeep',:recurse=>'true',:purge=>'true',:source=>['puppet:///modules/xldeploy/hotfix/45'],:path=>'/opt/xl-deploy/xl-deploy-server/hotfix'})}
      ...
      it { should contain_ini_setting('xldeploy.admin.password').with({:ensure=>'present',:path=>'/opt/xl-deploy/xl-deploy-server/conf/deployit.conf',:section=>'',:setting=>'admin.password',:value=>'xebialabs'})}
      ...
      it { should contain_xldeploy_setup('default')}
